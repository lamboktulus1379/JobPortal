...
func (host *HostStruct) HTTPPost() ([]byte, int, error) {
	...
	req, err := http.NewRequest("POST", host.Host+host.Endpoint, bytes.NewBuffer(dataByte))
	if err != nil {
...
func (host *HostStruct) Do(req *http.Request) ([]byte, int, error) {
	...
	fmt.Println("Request Host: ", host.Method, host.Host+host.Endpoint)
	...
	host.HTTPClient = &http.Client{Timeout: time.Second * 10, Transport: tr}
	...
	body, err := ioutil.ReadAll(host.HTTPResponse.Body)
	if err != nil {
		fmt.Println("Error network. ", err)
		return nil, 0, err
	}
	fmt.Println("response Status:", host.HTTPResponse.Status)
	fmt.Println("response Headers:", host.HTTPResponse.Header)
